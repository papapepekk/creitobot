...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `eae @${num.split('@')[0]}\ seja bem vindo/a ao grupo *${mdata.subject}* comportasse para nao ganhar ban`
				...
				teks = `sai fora @${num.split('@')[0]} se voltar vai levar bala`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: 'calma ae,to fazeno aqui',
				success: '️❬ ✔ ❭ pronto po,nem demorou',
				error: {
				...
				only: {
					...
					ownerG: '❬❗❭ COMANDO SÓ PODE SER USADO PELO Gustakk😜.❬❗❭ ',
					ownerB: '❬❗❭  COMANDO SÓ PODE SER USADO PELO Gustakk😜.❬❗❭  ',
